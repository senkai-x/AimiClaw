...
function mergeStreamingTextValue(previous: unknown, incoming: unknown): unknown {
  if (typeof incoming !== 'string') return incoming ?? previous;
  if (typeof previous !== 'string' || previous.length === 0) return incoming;
  if (incoming === previous || incoming.length === 0) return previous;
  if (incoming.startsWith(previous)) return incoming;
  if (previous.startsWith(incoming)) return previous;
  return `${previous}${incoming}`;
}

function mergeContentBlocks(previous: ContentBlock[], incoming: ContentBlock[]): ContentBlock[] {
  const merged = previous.map((block) => ({ ...block }));

  for (const rawBlock of incoming) {
    const block = { ...rawBlock } satisfies ContentBlock;
    const blockId = typeof block.id === 'string' && block.id ? block.id : null;

    if (block.type === 'tool_use' || block.type === 'toolCall') {
      const index = blockId
        ? merged.findIndex((candidate) => (candidate.type === 'tool_use' || candidate.type === 'toolCall') && candidate.id === blockId)
        : -1;
      if (index === -1) {
        merged.push(block);
      } else {
    ...
    if (block.type === 'text' || block.type === 'thinking') {
      const index = blockId
        ? merged.findIndex((candidate) => candidate.type === block.type && candidate.id === blockId)
        : [...merged].reverse().findIndex((candidate) => candidate.type === block.type && !candidate.id);
      const resolvedIndex = index >= 0 && !blockId ? merged.length - 1 - index : index;
      if (resolvedIndex === -1) {
        merged.push(block);
      } else {
        const existing = merged[resolvedIndex];
        merged[resolvedIndex] = {
          ...existing,
          ...block,
          text: block.type === 'text'
            ? mergeStreamingTextValue(existing.text, block.text) as string | undefined
            : existing.text,
          thinking: block.type === 'thinking'
            ? mergeStreamingTextValue(existing.thinking, block.thinking) as string | undefined
            : existing.thinking,
        };
      }
      continue;
...
function mergeToolCallArrays(previous: unknown, incoming: unknown): unknown {
  if (!Array.isArray(incoming)) return incoming ?? previous;
  if (!Array.isArray(previous) || previous.length === 0) return incoming;

  const merged = previous.map((call) => (call && typeof call === 'object') ? { ...(call as Record<string, unknown>) } : call);

  for (const rawCall of incoming as Array<Record<string, unknown>>) {
    if (!rawCall || typeof rawCall !== 'object') {
      merged.push(rawCall);
      continue;
    }
    const incomingId = typeof rawCall.id === 'string' && rawCall.id ? rawCall.id : null;
    const index = incomingId
      ? merged.findIndex((candidate) => candidate && typeof candidate === 'object' && (candidate as Record<string, unknown>).id === incomingId)
      : -1;
    ...
    const existing = merged[index] as Record<string, unknown>;
    const mergedFunction = (() => {
      const prevFn = existing.function;
      const nextFn = rawCall.function;
      if (!prevFn || typeof prevFn !== 'object' || !nextFn || typeof nextFn !== 'object') {
        return nextFn ?? prevFn;
      }
      const prevFnObj = prevFn as Record<string, unknown>;
      const nextFnObj = nextFn as Record<string, unknown>;
      return {
        ...prevFnObj,
        ...nextFnObj,
        arguments: mergeStreamingTextValue(prevFnObj.arguments, nextFnObj.arguments),
      };
    })();
...
function mergeStreamingMessage(previous: unknown, incoming: unknown): unknown {
  ...
  if (Array.isArray(prev.content) && Array.isArray(next.content)) {
    merged.content = mergeContentBlocks(prev.content as ContentBlock[], next.content as ContentBlock[]);
  } else if (typeof prev.content === 'string' && typeof next.content === 'string') {
    merged.content = mergeStreamingTextValue(prev.content, next.content);
  } else if (next.content == null) {
    merged.content = prev.content;
  }

  if ('text' in prev || 'text' in next) {
    merged.text = mergeStreamingTextValue(prev.text, next.text);
  }

  const prevToolCalls = prev.tool_calls ?? prev.toolCalls;
  const nextToolCalls = next.tool_calls ?? next.toolCalls;
  const mergedToolCalls = mergeToolCallArrays(prevToolCalls, nextToolCalls);
  if (mergedToolCalls != null) {
    if ('tool_calls' in prev || 'tool_calls' in next) merged.tool_calls = mergedToolCalls;
    if ('toolCalls' in prev || 'toolCalls' in next) merged.toolCalls = mergedToolCalls;
  }
