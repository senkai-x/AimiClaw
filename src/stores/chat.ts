...
export interface RawMessage {
  ...
  /** Local-only: selected skills for this message (not sent to/from Gateway) */
...
interface ChatState {
  ...
  /** Flag indicating user clicked "New Chat" and a new session should be created on first message */
...
const HISTORY_POLL_SILENCE_WINDOW_MS = 800;  // 0.8秒静默窗口（原2.5秒）
...
/**
 * Parse and remove skill prefixes from user messages loaded from Gateway history.
 * Extracts skills like /search-merchant from message content and stores them in _selectedSkills.
 */
function parseAndRemoveSkillPrefixes(messages: RawMessage[]): RawMessage[] {
  return messages.map((msg) => {
    ...
    // Match skill patterns at the start: /skill-name /another-skill text...
    const skillPattern = /^((?:\/[\w-]+\s+)+)/;
    const match = text.match(skillPattern);

    if (!match) return msg;

    // Extract skills from the prefix
    const skillsPrefix = match[1];
    const skills = skillsPrefix.trim().split(/\s+/).map(s => s.substring(1)); // Remove leading /

    // Remove prefix from content
    const cleanText = text.substring(skillsPrefix.length).trim();

    // Update message with clean text and extracted skills
    return {
      ...msg,
      content: cleanText,
      _selectedSkills: skills.length > 0 ? skills : undefined,
    };
  });
}
...
export const useChatStore = create<ChatState>((set, get) => ({
              ...
              sessionsToLabel.map(async (session) => {
                try {
                  ...
                  set((s) => {
                    ...
                    if (firstUser) {
                      let labelText = getMessageText(firstUser.content).trim();
                      // Remove skill prefixes from label
                      const skillPattern = /^((?:\/[\w-]+\s+)+)/;
                      const match = labelText.match(skillPattern);
                      if (match) {
                        labelText = labelText.substring(match[1].length).trim();
                      }
                      if (labelText) {
  ...
  newSession: () => {
    // Only set a flag to indicate a new session should be created on first message.
    // The actual session creation happens in sendMessage when the user sends their first message.
    const { currentSessionKey, messages, sessionLastActivity, sessionLabels } = get();
    // Only clear empty sessions when switching to a "new chat" state
  ...
  loadHistory: async (quiet = false) => {
    ...
    const loadPromise = (async () => {
      ...
      // Parse and remove skill prefixes from user messages
      const messagesWithSkills = parseAndRemoveSkillPrefixes(enrichedMessages);

      // Preserve the optimistic user message during an active send.
      // The Gateway may not include the user's message in chat.history
      // until the run completes, causing it to flash out of the UI.
      let finalMessages = messagesWithSkills;
      const userMsgAt = get().lastUserMessageAt;
      if (get().sending && userMsgAt) {
        const userMsMs = toMs(userMsgAt);
        const hasRecentUser = messagesWithSkills.some(
        ...
        if (!hasRecentUser) {
          ...
          if (optimistic) {
            finalMessages = [...messagesWithSkills, optimistic];
          }
        }
      }
      ...
      const { lastUserMessageAt, sending: isSendingNow } = get();
  ...
  sendMessage: async (
    ...
    // Prepend skills to message text
    let messageText = trimmed;
    if (skills && skills.length > 0) {
      const skillsPrefix = skills.map(skill => `/${skill}`).join(' ');
      messageText = `${skillsPrefix} ${trimmed}`;
    }
    ...
    const targetSessionKey = resolveMainSessionKeyForAgent(targetAgentId) ?? currentSessionKey;
    ...
    const userMsg: RawMessage = {
      ...
      _selectedSkills: skills && skills.length > 0 ? skills : undefined,
        ...
        result = await hostApiFetch<{ success: boolean; result?: { runId?: string }; error?: string }>(
          '/api/chat/send-with-media',
          {
            ...
            body: JSON.stringify({
              ...
              message: messageText || 'Process the attached file(s).',
              deliver: false,
              idempotencyKey,
      ...
      case 'final': {
        ...
        if (finalMsg) {
          ...
          set((s) => {
            ...
            if (alreadyExists) {
              // Message exists - just clear streaming state and end if this is final output
              if (hasOutput) {
                clearHistoryPoll();
                return {
                  streamingText: '',
            ...
            // Tool-only message - keep sending state
            return {
              messages: [...s.messages, msgWithImages],
              streamingText: '',
              streamingMessage: null,
          ...
          // No message in final event - just clear streaming state and end
          clearHistoryPoll();
