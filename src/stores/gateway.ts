...
const LOAD_SESSIONS_MIN_INTERVAL_MS = 800;   // 0.8秒（原1.2秒）
const LOAD_HISTORY_MIN_INTERVAL_MS = 400;    // 0.4秒（原0.8秒）
  ...
  if (phase === 'completed' || phase === 'done' || phase === 'finished' || phase === 'end') {
    import('./chat')
      .then(({ useChatStore }) => {
        ...
        // Do NOT load history here to avoid sudden jumps when displaying the complete conversation process
        // The final event in handleChatEvent will properly end the conversation
