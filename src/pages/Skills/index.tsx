          ...
          {filteredSkills.length === 0 ? (
            ...
            <>
              ...
              {filteredSkills.filter(s => s.enabled).length > 0 && (
                <div className="mb-8">
                  <h2 className="text-base font-medium text-foreground/80 mb-4">已安装</h2>
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                    {filteredSkills.filter(s => s.enabled).map((skill) => (
                      <div
                        key={skill.id}
                        className="group flex items-center justify-between p-4 rounded-xl hover:bg-black/5 dark:hover:bg-white/5 transition-all cursor-pointer border border-black/5 dark:border-white/10"
                        onClick={() => setSelectedSkill(skill)}
                      >
                        <div className="flex items-center gap-3 flex-1 overflow-hidden pr-4">
                          <div className="h-11 w-11 shrink-0 flex items-center justify-center text-2xl bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-950/30 dark:to-indigo-950/30 rounded-xl">
                            {skill.icon || '🧩'}
                          </div>
                          <div className="flex flex-col overflow-hidden flex-1 min-w-0">
                            <h3 className="text-[15px] font-medium text-foreground truncate mb-0.5">
                              {skill.name}
                            </h3>
              ...
              {/* Available/Disabled Skills Section */}
              {filteredSkills.filter(s => !s.enabled).length > 0 && (
                <div>
                  <h2 className="text-base font-medium text-foreground/80 mb-4">推荐</h2>
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                    {filteredSkills.filter(s => !s.enabled).map((skill) => (
                      <div
                        key={skill.id}
                      ...
                      >
                        <div className="flex items-center gap-3 flex-1 overflow-hidden pr-4">
                          <div className="h-11 w-11 shrink-0 flex items-center justify-center text-2xl bg-gradient-to-br from-purple-50 to-pink-50 dark:from-purple-950/30 dark:to-pink-950/30 rounded-xl">
                            {skill.icon || '📦'}
                          </div>
                          <div className="flex flex-col overflow-hidden flex-1 min-w-0">
                            <h3 className="text-[15px] font-medium text-foreground truncate mb-0.5">
                        ...
                        <button
                          className="shrink-0 h-8 w-8 rounded-full bg-foreground/5 hover:bg-foreground/10 dark:bg-white/5 dark:hover:bg-white/10 flex items-center justify-center transition-colors"
                          onClick={(e) => {
                            e.stopPropagation();
