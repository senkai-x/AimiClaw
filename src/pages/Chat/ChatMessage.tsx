...
import { Sparkles, Copy, Check, ChevronDown, ChevronRight, Wrench, FileText, Film, Music, FileArchive, File, X, FolderOpen, ZoomIn, Loader2, CheckCircle2, AlertCircle, Search, Phone } from 'lucide-react';
...
import loadingThinkingGif from '@/assets/loading-thinking.gif';
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Selected Skills Badges (for user messages) */}
        {isUser && message._selectedSkills && message._selectedSkills.length > 0 && (
          <div className="flex flex-wrap gap-1.5">
            {message._selectedSkills.map((skillId) => {
              const skillInfo: Record<string, { label: string; icon: typeof Search }> = {
                'search-merchant': { label: '商家搜索', icon: Search },
                'instruction_generator_uploader': { label: '指令生成', icon: FileText },
                'outbound_call': { label: '外呼', icon: Phone },
              };
              const skill = skillInfo[skillId] || { label: skillId, icon: Wrench };
              const Icon = skill.icon;
              return (
                <div
                  key={skillId}
                  className="inline-flex items-center gap-1 rounded-full border border-blue-500/20 bg-blue-500/10 px-2.5 py-0.5 text-[12px] font-medium text-blue-600 dark:text-blue-400"
                >
        ...
        {/* Loading animation below text bubble when streaming */}
        {!isUser && isStreaming && hasText && (
          <div className="flex items-center justify-center">
            <img
              src={loadingThinkingGif}
              alt="Loading..."
...
function ThinkingBlock({ content }: { content: string }) {
  ...
  return (
    <div className="w-full rounded-xl border border-purple-500/20 bg-purple-500/5 dark:bg-purple-500/10 text-[14px]">
      <button
        className="flex items-center gap-2 w-full px-3 py-2 text-purple-700 dark:text-purple-300 hover:text-purple-900 dark:hover:text-purple-100 transition-colors"
        onClick={() => setExpanded(!expanded)}
      >
        {expanded ? <ChevronDown className="h-3.5 w-3.5" /> : <ChevronRight className="h-3.5 w-3.5" />}
        <Sparkles className="h-3.5 w-3.5" />
        <span className="font-medium">思考过程</span>
        <span className="ml-auto text-xs opacity-60">{expanded ? '收起' : '展开'}</span>
      </button>
      {expanded && (
        <div className="px-3 pb-3 text-foreground/80">
...
function ToolCard({ name, input }: { name: string; input: unknown }) {
  ...
  return (
    <div className="rounded-xl border border-blue-500/20 bg-blue-500/5 dark:bg-blue-500/10 text-[14px]">
      <button
        className="flex items-center gap-2 w-full px-3 py-2 text-blue-700 dark:text-blue-300 hover:text-blue-900 dark:hover:text-blue-100 transition-colors"
        onClick={() => setExpanded(!expanded)}
      >
        <Wrench className="h-3.5 w-3.5 shrink-0" />
        <span className="font-medium text-sm">工具调用：{name}</span>
        {expanded ? <ChevronDown className="h-3 w-3 ml-auto" /> : <ChevronRight className="h-3 w-3 ml-auto" />}
      </button>
      {expanded && input != null && (
        <div className="px-3 pb-2">
          <pre className="text-xs text-foreground/70 overflow-x-auto bg-black/5 dark:bg-white/5 rounded-lg p-2">
