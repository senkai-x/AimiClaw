...
interface ChatInputProps {
  onSend: (text: string, attachments?: FileAttachment[], targetAgentId?: string | null, skills?: string[]) => void;
  onStop?: () => void;
  disabled?: boolean;
  sending?: boolean;
  isEmpty?: boolean;
...
/**
 * Map UI skill IDs to backend skill command names.
 * The UI uses display names while the backend expects the actual skill file names.
 */
const skillCommandMap: Record<string, string> = {
  'search-public-merchants': 'search-merchant',
  'merchant-detail': 'merchant-profile',
  'business-profile': 'merchant-analytics',
  'create-opportunity': 'multi-channel-invite',
  'ai-call': 'outbound_call',
};
...
export function ChatInput({ onSend, onStop, disabled = false, sending = false, isEmpty = false, selectedSkills = [], onRemoveSkill }: ChatInputProps) {
  ...
  const handleSend = useCallback(() => {
    ...
    // Map UI skill IDs to backend command names
    const skillsToSend = selectedSkills.length > 0
      ? selectedSkills.map(skillId => skillCommandMap[skillId] || skillId)
      : undefined;
    console.log(`[handleSend] text="${textToSend.substring(0, 50)}", attachments=${attachments.length}, ready=${readyAttachments.length}, sending=${!!attachmentsToSend}, skills=${skillsToSend?.join(',')}`);
    ...
    onSend(textToSend, attachmentsToSend, targetAgentId, skillsToSend);
    setTargetAgentId(null);
    setPickerOpen(false);
  }, [input, attachments, canSend, onSend, targetAgentId, selectedSkills]);
