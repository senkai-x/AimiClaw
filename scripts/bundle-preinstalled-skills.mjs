...
if (existsSync(LOCAL_SKILLS_DIR)) {
  ...
  for (const entry of entries) {
    ...
    if (stat.isDirectory() && existsSync(skillFile)) {
      const targetPath = join(OUTPUT_ROOT, entry);
      cpSync(sourcePath, targetPath, { recursive: true });
      echo`   Copied local skill: ${entry}`;
      copiedCount++;
      lock.localSkills.push({
        slug: entry,
        source: 'local'
      });
    }
  ...
  if (copiedCount > 0) {
    echo`   Copied ${copiedCount} local skill(s)`;
  }
}

if (process.env.SKIP_PREINSTALLED_SKILLS === '1') {
  echo`⏭  SKIP_PREINSTALLED_SKILLS=1 set, skipping remote skills fetch.`;
