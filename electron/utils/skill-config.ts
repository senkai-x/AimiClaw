...
interface PreinstalledLockFile {
    ...
    localSkills?: PreinstalledLockEntry[];
}
...
async function readPreinstalledLockVersions(sourceRoot: string): Promise<Map<string, string>> {
    ...
    try {
        ...
        // Process local skills (use slug as version since local skills don't have git commit versions)
        for (const entry of parsed.localSkills || []) {
            const slug = entry.slug?.trim();
            if (slug) {
                // Use 'local' as version identifier for local skills
                versions.set(slug, 'local');
            }
        }
        return versions;
    } catch (error) {
...
/**
 * Read all preinstalled skills from lock file (both remote and local)
 */
async function readAllPreinstalledSkillsFromLock(sourceRoot: string): Promise<PreinstalledSkillSpec[]> {
    const lockPath = join(sourceRoot, '.preinstalled-lock.json');
    if (!existsSync(lockPath)) {
        return [];
    }
    try {
        const raw = await readFile(lockPath, 'utf-8');
        const parsed = JSON.parse(raw) as PreinstalledLockFile;
        const allSkills: PreinstalledSkillSpec[] = [];

        // Add remote skills from lock file
        for (const entry of parsed.skills || []) {
            if (entry.slug?.trim()) {
                allSkills.push({
        ...
        // Add local skills from lock file
        for (const entry of parsed.localSkills || []) {
            if (entry.slug?.trim()) {
                allSkills.push({
                    slug: entry.slug.trim(),
...
export async function ensurePreinstalledSkillsInstalled(): Promise<void> {
    ...
    // Also read local skills from lock file
    const lockSkills = await readAllPreinstalledSkillsFromLock(sourceRoot);
    const allSkills = [...skills, ...lockSkills];
